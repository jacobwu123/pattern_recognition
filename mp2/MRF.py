...
def MRF(I, J, eta, zeta, beta):
    ...
    for pixel_y_coordinate in y:
        for pixel_x_coordinate in x:
            leastEnergy = energy_evaluation(
                I, denoised, pixel_x_coordinate,
                pixel_y_coordinate, 1, eta, zeta, beta)
            temp = energy_evaluation(
                I, denoised, pixel_x_coordinate,
                pixel_y_coordinate, -1, eta, zeta, beta)
            if leastEnergy > temp:
...
def energy_evaluation(I, J, pixel_x_coordinate, pixel_y_coordinate,
    ...
    if pixel_y_coordinate - 1 >= 0:
        latent_latent += J[pixel_y_coordinate][pixel_x_coordinate] * \
            J[pixel_y_coordinate - 1][pixel_x_coordinate]
    if pixel_x_coordinate + 1 < x_size:
        latent_latent += J[pixel_y_coordinate][pixel_x_coordinate] * \
            J[pixel_y_coordinate][pixel_x_coordinate + 1]
    ...
    observed_latent = J[pixel_y_coordinate][
        pixel_x_coordinate] * I[pixel_y_coordinate][pixel_x_coordinate]
...
def f_reconstruction_error(original_image, reconstructed_image):
    ...
    for y in range(y_size):
        for x in range(x_size):
            reconstruction_error += (original_image[y]
                                     [x] - reconstructed_image[y][x])**2
